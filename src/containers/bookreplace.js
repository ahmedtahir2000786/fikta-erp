...
class BookReplace extends Component {
  constructor(props) {
    super(props);
    this.state = {
      name: "",
    ...
    if (
      ...
      if (this.state.setCnsg == false) {
        if (this.state.amount >= 5) {
          ...
          axios(config)
            .then((response) => {
              ...
              const obj = {
                ...
                address:
                  "(pehle parcel lena h phr dena h) " + this.state.address,
          ...
          this.setState({
            amounterror: "Amount must be greater than 5",
            amount: 0,
          });
        }
      } else {
        if (this.state.amount >= 5) {
          ...
          const obj = {
            ...
            cust_id: this.state.id,
            phone: this.state.phone,
            address: "(pehle parcel lena h phr dena h) " + this.state.address,
            amount: this.state.amount,
            color: this.state.color,
            size: this.state.size,
            design_name: this.state.designName,
                    ...
                    <td colSpan={3}>
                      <div className="input-group">
                        ...
                        <input
                          required
                          type="text"
                          placeholder="Address (Pehle lena h phr dena h)"
...
export default connect(mapStateToProps, mapDispatchToProps)(BookReplace);
