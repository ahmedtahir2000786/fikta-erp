...
if(isset($postdata) && !empty($postdata)){
   ...
   print_r ($city);
     ...
     $sql = "INSERT INTO `orders`(`consignmentnum`,`city`, `cust_id`, `name`, `phone`, `address`, `amount`, `color`, `size`, `design_name`, `status`, `reason`, `date`, `delete_ord`, `category`) VALUES ('{$consg}','{$city}','{$custid}','{$name}','{$phone}','{$address}', '{$amount}', '{$color}', '{$size}','{$design_name}', 'Booked', 'None', CONVERT_TZ(CURRENT_TIMESTAMP(), '+00:00','+11:00'), false, '{$catg}')";
     //echo $sql;
