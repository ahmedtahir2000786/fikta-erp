...
if ($searchby == "date") {
    $tdate = $_GET['todate'];
    $fdate = $_GET['fromdate'];
    $sql = "SELECT * FROM orders WHERE (status = 'Return') AND `date` BETWEEN '$fdate 00:00:00' AND '$tdate 23:59:59'";
} else if($searchby == "cnsg") {
    $cnsg =(int) $_GET['cnsg'];
    $sql = "SELECT * FROM orders WHERE (consignmentnum = $cnsg) AND (status = 'Return')";
}else if($searchby == "name") {
    $cnsg = $_GET['cnsg'];
    $sql = "SELECT * FROM orders WHERE (name = '{$cnsg}') AND (status = 'Return')";
}else if($searchby == "phone") {
    $cnsg =(int) $_GET['cnsg'];
    $sql = "SELECT * FROM orders WHERE (phone = $cnsg) AND (status = 'Return')";
}
    ...
    if($result = mysqli_query($con, $sql)){
        ...
        while($row=mysqli_fetch_assoc($result)){
            ...
            $orders[$cr]['returnRcvd'] = $row['returnRcvd'];
